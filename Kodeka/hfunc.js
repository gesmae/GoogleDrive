  ...
  //Built in function
    try {
            if (!window.openDatabase) {
                    alert(NSB._['SQLite not supported.']);
            } else {
                    if (typeof(shortName) == 'undefined') {
                        alert(NSB._["Database name required."]);
                        return 0;
                        }
            ...
            alert(NSB._["Invalid database version: {version}"].supplant({"version": version}));
            } else {
            alert(NSB._["Unknown error: {error}"].supplant({"error": e}));
            }
            return 0;
    }
    return myDB;
...
NSB.addProperties = function(ctrl, actualCtrl) {
    ...
    if (ctrl.getAttribute('data-nsb-type') === 'TextBox_jqm') { textBoxCtrl = ctrl; }
    ...
    NSB.defineProperty(ctrl, 'Width', {
        set: function(n) { actualCtrl.style.width = n + (typeof n === 'number' ? 'px' : ''); },
        get: function() { return actualCtrl.offsetWidth; }
    });
    NSB.defineProperty(ctrl, 'Height', {
        set: function(n) { textBoxCtrl.style.height = n + (typeof n === 'number' ? 'px' : ''); },
    ...
    if (ctrl.nodeName === 'LABEL') {
        NSB.defineProperty(ctrl, 'Caption', {
            ...
            get: function() { return actualCtrl.textContent; }
        });
    }
    if (ctrl.nodeName === 'INPUT' || ctrl.nodeName === 'TEXTAREA') {
        NSB.defineProperty(ctrl, 'text', {
            set: function(n) { actualCtrl.value = n; },
            get: function() { return actualCtrl.value; }
        });
    }
    ...
    ctrl.show = function() {if (actualCtrl.parentNode.id==actualCtrl.id + "_scroller") {
        console.log("show parent " + actualCtrl.id);
        actualCtrl.parentNode.style.display = 'block'}
        else {console.log("show " + actualCtrl.id);actualCtrl.style.display = 'block'}};
        
    ctrl.resize = function(l, t, w, h) { console.log(l & "," & t);ctrl.Left = l; ctrl.Top = t; ctrl.Width = w; ctrl.Height = h; };
};
...
NSB.Print=function(s){
  ...
  if(typeof(NSB.Printobj)!='undefined' && NSB.Printobj != null){
    ...
    NSB.Printobj.innerHTML=
      ...
      "</div>" +
      "<div onclick='NSB_Progress.style.display=\"none\"' class='NSB_ProgressClose'>\u00D7</div>"
...
NSB._sqlError = function(lastSQL) {
        ...
        return function(tx, err) {
            alert(NSB._['SQLite error: {errmsg} (Code {errcode}) {sql}'].supplant({"errmsg": err.message, "errcode": err.code, "sql": lastSQL}));
            return false;
        };
    }
...
Dialog.prototype = {
    ...
    _makeButtons: function(buttons) {
        ...
        for (var buttonText in buttons) {
            ...
            bindEventHandler(button, 'touchstart', this._makeHandler(buttons[buttonText], this));
...
if (typeof document !== 'undefined') {
    var AppBuildStamp='2013';
    var AppVersion='1';
    var AppLegalCopyright='Gesmae Informatica';
    var NSBVersion='3';
